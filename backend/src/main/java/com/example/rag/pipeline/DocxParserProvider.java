...
@Component
final class DocxParserProvider implements ParserProvider {

    private final OfficeDocumentParser parser;

    DocxParserProvider(OfficeDocumentParser parser) {
        this.parser = parser;
    }

    @Override
    public ParserProviderKind provider() {
        return ParserProviderKind.DOCX_POI;
    }

    @Override
    public Set<DocumentFormat> supportedFormats() {
        return Set.of(DocumentFormat.DOCX);
    }
